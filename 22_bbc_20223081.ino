...
// Servo adjustment
#define _DUTY_MAX 2500  // Servo angle: 55 degree
#define _DUTY_NEU 1500  // Servo angle: 0 degree
#define _DUTY_MIN 500   // Servo angle: -89 degree
#define _SERVO_ANGLE_DIFF 144 // Replace with |144| degree
...
void loop() {
  ...
  if(event_dist) {
    ...
    if(dist_target > dist_ema) { // Replace 1 with a proper test expression using dist_target and dist_ema.
      duty_target = _DUTY_NEU + _BANGBANG_RANGE + 150; // Complete this line using _BANGBANG_RANGE.
      digitalWrite(PIN_LED, 0);
    }
    else {
      duty_target = _DUTY_NEU; // Complete this line using _BANGBANG_RANGE.
      duty_target = _DUTY_NEU - _BANGBANG_RANGE -50;
...
// Replace the below line with the equation obtained from nonlinear regression analysis
  return 967 - 4.9 * a_value + 8.98 * pow(0.1, 3) * pow(a_value, 2) - 5.67 * pow(0.1, 6) * pow(a_value, 3);
}
